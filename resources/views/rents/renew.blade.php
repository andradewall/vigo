<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-8 space-y-6">
            <x-form post :action="route('rents.renew', $rent)" with-loading>
                        ...
                        <div class="flex">
                            <div id="all-procuts-wrapper" class="w-full relative">
                                <x-form.select-searchable
                                        ...
                                        required
                                        :list="$allProducts->map(function ($product) {
                                        return [
                                            'value' => $product->id,
                            ...
                            @foreach($rentProducts as $product)
                                <div class="item-{{ $product->id }}">
                                    <input type="text"
                                           ...
                                           x-on:blur="calculate"
                                           class="product-selected bg-gray-50 read-only:bg-gray-200 border
                                           border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    />
                                </div>
                                <div class="item-{{ $product->id }} flex items-center justify-between">
                                    <span>
                                        {{ $product->type->code_prefix }}-{{$product->code}} |
                                        {{ $product->type->name }}
                                    </span>
                                    <button type="button" id="delete-item-{{ $product->id }}"
                                            data-relative-id="item-{{ $product->id }}"
                                            x-on:click="removeProduct($event.currentTarget)">
